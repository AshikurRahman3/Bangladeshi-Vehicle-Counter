...
while True:
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if conf > 0.8:
                # Display class name, ID, and confidence in a single putText call
                label_text = f'{currentClass}'
                # cv2.putText(img, label_text, (max(0, x1), max(35, y1)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255),
                #             2)
    ...
    for result in resultsTracker:
        ...
        cvzone.putTextRect(img, f' {int(id)}', (max(0, x1),  max(35, y1 + 20)),
                           scale=1, thickness=1)
        # cv2.putText(img,f' {int(id)}', (max(0, x1),max(35, y1 + 20)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0),
        #             1)
